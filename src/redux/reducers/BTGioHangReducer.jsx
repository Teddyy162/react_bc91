...
// export const BTGioHangReducer = (state = initialState, action) => {
//     switch (action.type) {
...
//             */
//             let spGioHangClick = {...action.payload, soLuong:1};
//             //Kiểm tra spGioHangClick có trong state.gioHang hay chưa
//             let spGH = state.gioHang.find(sp=>sp.maSP === spGioHangClick.maSP);
//             if(spGH){
//                 spGH.soLuong += 1;
//             }else {
...
//             return {...state,gioHang:[...state.gioHang]};
...
const BTGioHangReducer = createSlice({
    name: 'BTGioHangReducer', //Giúp tạo ra tên action (mình không cần phải hardcode action)
    initialState,
    reducers: {
        ...
        thayDoiSoLuongAction: (state, action) => {
            ...
            const spGH = state.gioHang.find(sp => sp.maSP == maSP);
            if(spGH){
...
export const { themGioHangAction, xoaGioHangAction, thayDoiSoLuongAction } = BTGioHangReducer.actions

export default BTGioHangReducer.reducer //nó là function (state, action) => state